...
安装默认改为 conda 命名环境 `qwen3-omni`，遵循 conda 的环境目录配置；仓库内路径不再作为默认安装位置。新增测试覆盖默认名称、自定义名称、已有环境与目录保护、显式路径和参数冲突，全部 42 项测试通过。实际确认安装脚本拒绝覆盖本机已有环境，`conda run -n qwen3-omni` 正常定位原环境；本次未重新安装依赖或启动 GPU 服务。
